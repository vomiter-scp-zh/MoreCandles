...
public interface IEndCandle {
    private float uCurveRandom(RandomSource random){
        float u = random.nextFloat();
        float v = random.nextFloat();
        return Math.min(u, v);
    }
    Iterable<Vec3> getEndCandleParticleOffsets(BlockState p_152812_);


    default void animateEndTick(BlockState state, @NotNull Level level, @NotNull BlockPos pos, @NotNull RandomSource random) {
        if (!state.getValue(LIT)) return;

        // CandleBlock 會依 CANDLES 回傳對應的 offsets
        for (Vec3 off : getEndCandleParticleOffsets(state)) {
